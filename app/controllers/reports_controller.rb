  ...
  def search(provisionStatusId)
    ...
    if (params[:device_name] !="Enter device name" && params[:device_address] !="State / Location" && params[:device_group] !="")
      ...
      @all_devices.each do |device|
        if device.latest_gps_reading && device.latest_gps_reading.short_address.downcase =~ /#{params[:device_address].downcase}/
          @devices << device
        end
      end
    ...
    if params[:reports_within] !=""
      ...
      end_date = Date.today
      temp_storage_for_devices=[]
      @devices.each do |device|
        if device.latest_gps_reading && device.latest_gps_reading.created_at.to_date >= start_date && device.latest_gps_reading.created_at.to_date <= end_date
          temp_storage_for_devices << device
        end
      end
      @devices = temp_storage_for_devices
    end
  end
