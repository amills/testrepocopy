...
class ReportsController < ApplicationController
  ...
  def all      
     ...
     @readings = Reading.find(:all,  :include => "device",
                              :conditions => ["device_id = ? and account_id = ? and date(readings.created_at) between ? and ?", 
     ...
     @record_count = Reading.count('id', :include => "device", 
                                   :conditions => ["device_id = ? and account_id = ? and date(readings.created_at) between ? and ?", params[:id], session[:account_id], @start_time, @end_time])
    ...
    stream_csv do |csv|
     ...
     if params[:type] == 'stop'
       stop_events.each do |stop_event|                              
            ...
            csv << [address, ((stop_event.duration.to_s.size > 0) ? stop_event.duration : 'Unknown'), local_time, stop_event.latitude, stop_event.longitude]
        end
     else
