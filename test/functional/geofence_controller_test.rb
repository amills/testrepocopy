...
class GeofenceControllerTest < Test::Unit::TestCase
  ...
  def test_detail
    get :detail, {:id => "1"}, { :user => users(:testuser), :account_id => "1" }
    assert_not_nil assigns("geofence")
  end

   def test_detail_for_invalid_geofence_id
    get :detail, {:id => "5"}, { :user => users(:testuser), :account_id => "1" }
    assert_response 302
    assert_equal flash[:error], "Invalid action."
   end
   
  def test_view
    get :view, {:id => "account1"}, { :user => users(:testuser), :account_id => "1" }
    assert_not_nil assigns("account")
    
    get :view, {:id => "device1"}, { :user => users(:testuser), :account_id => "1" }
    assert_not_nil assigns("device")
    
    get :view, {:id => "1"}, { :user => users(:testuser), :account_id => "1" }
    assert_not_nil assigns("device")
  end
  
  def test_new
    post :new, {:id => "1", :name => "qwerty", :bounds=>"1,1,1", :address=>"1600 Penn Ave"}, { :user => users(:testuser), :account_id => "1" }
    assert_equal flash[:success] , "qwerty created succesfully"
  end
  
  def test_invalid_device
     post :new, {:id => "1", :name =>"", :bounds=>"1,1,1", :address=>"1600 Penn Ave"}, { :user => users(:testuser), :account_id => "1" }
  ...
  def test_delete
    get :delete, {:id => '1', :device_id => '1'}, { :user => users(:testuser), :account_id => "1" }
  ...
  def delete_unknown_geofence
    get :delete, {:id => '17521'}, { :user => users(:testuser), :account_id => "1" }
    assert_equal "Invalid action.", flash[:error]
    assert_redirected_to :controller => "geofence", :action => "index"        
  end
  
  def test_for_device_id_for_delete
    get :delete, {:id => '3'}, { :user => users(:testuser), :account_id => "1" }
    assert_equal "work deleted successfully", flash[:success]
    assert_redirected_to :controller => "geofence", :action => "index"
  end
  
  def test_edit
    post :edit, {:device_id => '1', :id => '1', :name => "qwerty", :bounds=>"1,1,1", :address=>"1600 Penn Ave", :ref_url=>"/geofence/index"}, { :user => users(:testuser), :account_id => "1" }
    assert_equal flash[:success] , 'qwerty updated succesfully'
    assert_redirected_to :controller => "geofence", :action => "index"
  end
  
  def test_edit_invalid_geofence
      post :edit, {:device_id => '1', :id => '1', :name => "", :bounds=>"1,1,1", :address=>"1600 Penn Ave", :ref_url=>"/geofence/index"}, { :user => users(:testuser), :account_id => "1" }
      assert flash[:error] 
  end
  
  def test_edit_for_invalid_geofence
      get :edit, {:id=>'5'}, { :user => users(:testuser), :account_id => "1" }
      assert_response 302
      assert_equal flash[:error], "Invalid action."
  end
  ...
  def  test_after_edit_check_nil_for_device_id
      post :edit, {:id=>'1', :device=>'1', :radio=>'2',:account_id=>'1', :name => "qwerty", :bounds=>"1,1,1", :address=>"1600 Penn Ave", :ref_url=>"/geofence/index"}, { :user => users(:testuser), :account_id => "1" }      
  ...
  def test_view_gf_is_true_for_device
     get :view, {:id=>"device1", :gf=>'1'}, { :user => users(:testuser), :account_id => "1" }
     assert_response :success        
  end
  ...
  def test_view_for_invalid_device_id
     get :view, {:id=>'456'}, { :user => users(:testuser), :account_id => "1" }     
     assert_response 302                
     assert_equal flash[:error], "Invalid action."
  end
  
  def test_view_for_invalid_gf_for_device_level
     get :view, {:id=>"device8", :gf=>'5'}, { :user => users(:testuser), :account_id => "1" }
     assert_response 302
     assert_equal flash[:error], "Invalid action"
  end
    
  def test_view_gf_is_true_for_account
      get :view, {:id=>"account1", :gf=>'20'}, { :user => users(:testuser), :account_id => "1" }
      assert_response :success
  end    
  
  def test_view_detail
      get :view_detail, {:id=>"1"}, { :user => users(:testuser), :account_id => "1" }
      assert_response :success
  end
