...
class ReadingsControllerTest < Test::Unit::TestCase
  ...
  def test_recent
    get :recent, {}, {:user=>users(:dennis), :account_id => accounts(:app).id}
  ...
  def test_last
    ...
    get :last, { :id => "1"}, {:user => users(:dennis), :user_id => users(:dennis), :account_id => accounts(:app)}
  ...
  def test_all
    @request.host = "app.ublip.com"
     @request.env["Authorization"] = "Basic " + Base64.encode64("dennis@ublip.com:testing")
     get :all, {}, {:user => users(:dennis), :user_id => users(:dennis), :account_id => accounts(:app)}
  ...
  def test_last_not_auth
    @request.host="app.ublip.com"
    @request.env["Authorization"] = "Basic " + Base64.encode64("dennis@ublip.com:testing")
    get :last, { :id => 7}, {:user => users(:dennis), :user_id => users(:dennis), :account_id => accounts(:app)}
