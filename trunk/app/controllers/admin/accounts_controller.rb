class Admin::AccountsController < ApplicationController
  ...
  def encode_account_options(account)
    (account.show_runtime ? "R" : "-") + (account.show_statistics ? "S" : "-") + (account.show_maintenance ? "M" : "-") + (account.show_notifications ? "N" : "-")
end
...
def apply_options_to_account(params,account)
    update_attributes_with_checkboxes(account,[:show_runtime,:show_statistics,:show_maintenance,:show_notifications],params[:options])
  end
