module HomeHelper
  ...
  def decide_action 
	 ...
	 elsif @from_notifications
	 content << %(select_action(this,'from_notifications'))
  ...
  def show_device(device)
    ...
    if device.profile.is_fixed_asset
      #content << %(#{device.latitude}, #{device.longitude}, adsfasdfasf)
    elsif device.latest_gps_reading
...
content << %(</td>
  ...
  def show_notifications(device)
	...
	last_notification = Notification.find(:last, :conditions => ["user_id = ? and device_id = ?", session[:user_id], device.id])
#	if !device_notifications.nil?
#		last_notification = device_notifications
#	end
    ...
    if last_notification.nil?
      content << %(<td><a title="No active notifications.">No notifications for device</td><td>-</td><td>-</td>)
		...
		if last_notification.user_id == session[:user_id]
			content << %(<td><a title="Review notification history." href="/reports/notifications/#{device.id}">#{last_notification.notification_text or last_notification.notification_type}</td>)
			content << %(<td><script>document.write(displayLocalDT(#{last_notification.created_at.to_i*1000}));</script></td>)
	  else
